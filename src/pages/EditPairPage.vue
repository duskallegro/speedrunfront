...
export default {
  ...
  async beforeCreate() {
    ...
    let card = await WordPairsService.getCardById(this.$route.params.cardId);
  ...
  methods: {
    async editPairAction(models) {
      ...
      let result =  await WordPairsService.editPair(this.oldForeignWord,
              foreign, translation, padId);
      console.log(result);

      if (result)  {
        this.$router
            .push({ path: '/user/' + AuthService.getLoggedInUser().username + '/pad/' +
                  padId})
            .then(() => { this.$router.go() })
      } else  {
        return result;
